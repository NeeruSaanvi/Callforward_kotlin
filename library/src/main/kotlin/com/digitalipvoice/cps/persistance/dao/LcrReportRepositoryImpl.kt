...
class LcrReportRepositoryImpl : LcrReportRepositoryCustom {
    ...
    override fun searchLcrReportByUserId(query: TableQuery, userId: Long): TableResult {
        ...
        // columns to select
        val cols = arrayOf("u.id", "u.user_id", "u.created_at", "u.updated_at", "u.name", "u.default_carrier", "u.description", "u.average_rate", "GROUP_CONCAT(rd.name SEPARATOR ', ') as names", "GROUP_CONCAT(rd.carrier SEPARATOR ', ') as carriers")
        val table = """
             $table u
