...
class SMSRequestManager {
     ...
     */
    private val requestProcessingMap = mutableMapOf<String, SMSRequestManager.RequestStatus>()
     ...
     */
    private var requestDoneHandlers = mutableMapOf<String, () -> Unit>()
    ...
    fun registerSMSRequests(requestId:String, correlationIds:Collection<String>, timeOut:Long) {
        if (timeOut < 0) {
     ...
     */
    fun setSMSMessageReceived(requestId: String, correlationId: String){
        rpExecutorService.run {
     ...
     */
    fun addRequestDoneHandler(requestId: String, handler:() -> Unit) {
     ...
     * Remove
     */
    fun removeRequestDoneHandler(requestId: String) {
        rpExecutorService.run {
            requestDoneHandlers.remove(requestId)
        }
    }
